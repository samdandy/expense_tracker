#ifndef EXPENSE_H
#define EXPENSE_H#include <string>
...
class Expense {
  ...
  public: Expense(const string & expense_name,
    const string & expense_date, double expense_amount): name(expense_name),
  date(expense_date),
...
void add_expense_prompt(vector < Expense > & expenses, bool delete_mode = false) {
  while (true) {
    string name;
    string date;
    double amount;
    ...
    cin >> date;
    if (delete_mode) {
      expenses.emplace_back(name, date, 0.0);
      continue;
    }
...
void save_expenses_to_file(vector < Expense > & expenses,
  const string & file_name = "", string write_type = "append") {
  string file_name_to_use = file_name.empty() ? enter_file_name() : file_name;
  if (write_type == "overwrite") {
    fstream file;
    file.open(file_name_to_use, ios::out | ios::trunc);
    file.close();
  }
  add_file_header(file_name_to_use);
  for (const Expense & exp: expenses) {
    cout << "Saving expense: " << exp.get_name() << ", " << exp.get_date() << ", " << exp.get_amount() << endl;
    write_to_file(file_name_to_use, exp);
  }
  expenses.clear();
}
    ...
    if (vals.size() == 3) {
      ...
      for (const Expense & exp: expenses) {
        if (exp.get_name() == name && exp.get_date() == date) {
          found = true;
          new_expenses.push_back(exp);
          break;
        }
      }
      if (!found) {
        new_expenses.push_back(Expense(name, date, amount));
      }
    }
  }
  file.close();
  save_expenses_to_file(new_expenses, file_name, "overwrite");
  expenses.clear();
}
...
void delete_expense(const string & file_name, vector < Expense > & expenses) {
  ...
  getline(file, header_line);
  string line;
  vector < Expense > updated_expenses;
  while (getline(file, line)) {
    vector < string > vals = split(line, ',');
    if (vals.size() == 3) {
      string name = vals[0];
      ...
      if (to_delete) {
        ...
        updated_expenses.push_back(Expense(name, date, parse_amount(line)));
      }
    }
  }
  file.close();
  save_expenses_to_file(updated_expenses, file_name, "overwrite");
}
...
void delete_task() {
  ...
  add_expense_prompt(expenses, true);
  string file_name = enter_file_name();
